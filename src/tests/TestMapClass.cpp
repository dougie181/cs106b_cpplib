...
static void testInsertionOperator(Map<string,string> & elements,
                                  string pattern);
static void testExtractionOperator();
static void testMapCopy(Map<string,string> & map,
...
void testMapClass() {
   ...
   trace(foreach (string key in elements) pattern += key);
   ...
   test(elements.toString(),
        "{Be:Beryllium, H:Hydrogen, He:Helium, Li:Lithium}");
   testInsertionOperator(elements,
                         "Be:Beryllium, H:Hydrogen, He:Helium, Li:Lithium");
