...
public:
   ...
   virtual void scanArcData(TokenScanner & scanner, MyArc *forward,
                                                    MyArc *backward) {
      double cost = 1;
      ...
      forward->cost = cost;
      if (backward != NULL) backward->cost = cost;
   }
...
static void deleteArcsWithCost(MyGraph & g, double cost) {
   ...
   foreach (MyArc *arc in g.getArcSet()) {
      if (arc->cost == cost) {
         toRemove.add(arc);
      }
   }
   foreach (MyArc *arc in toRemove) {
      g.removeArc(arc);
   }
}
